...
namespace DictaNakdanVsto
{
    public partial class ThisAddIn
    {
        ...
        private void Application_WindowBeforeRightClick(Word.Selection Sel, ref bool Cancel)
        {
            ...
            // מחיקה אגרסיבית של כל כפתור ישן שהיה אי פעם כדי למנוע כפילויות
            for (int i = contextMenu.Controls.Count; i >= 1; i--)
            {
                var ctrl = contextMenu.Controls[i];
                if (ctrl.Caption != null && ctrl.Caption.Contains("נקדן דיקטה"))
                {
                    ctrl.Delete();
                }
            }
            ...
            // שימוש בלוגו גדול יותר (64) שוורד יכווץ יפה וברור למסך
            try
            {
                customButton.Picture = PictureDispConverter.ToIPictureDisp(Properties.Resources.icon_64);
            }
            catch { }
